...
# read in data 
data_mouse <- read_csv("filtered_manvsML_C9_8Weeks_gen_corr_filtered_redo_3.csv", col_types = cols (
  mouse_id = 'f',
...
ME_EXPL<- lmer(Duration_sniffing~Source*Genotype*(Odour_type_presentation + Sex) + (1 | mouse_id),data = data_mouse)
summary(ME_EXPL)
resid_panel(ME_EXPL)
...
#post-hoc exploring genotype at every sex and source
emm1 <- emmeans(ME_EXPL, ~Genotype*Sex|Source)
str(emm1)
coef_t_freq1 <- coef(pairs(emm1))
custom_contrasts1 <- list(
...
# Save the plot
ggsave("ManvsML_C9_8weeks_filtered_gen_3.tiff", plot = pEXPL_Gen_Sex, width = 5, height = 3, dpi = 300)
