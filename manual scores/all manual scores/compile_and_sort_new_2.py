...
for file_ in allFiles:
    ...
    ### GET Odour1.1 
    # set index selection based on text
    index_start = df[df.iloc[:,0]=='Odour2.1'].index.values.astype(int)[0]  
    # select the data between set indexes
    df_21 = df.query('index == 8')
    
    ### GET Odour1.2
    # set index selection based on text
    index_start2 = df[df.iloc[:,0]=='Odour2.2'].index.values.astype(int)[0]  
    # select the data between set indexes
    df_22 = df.query('index == 9')    
    
    ### GET Odour1.3
    # set index selection based on text
    index_start3 = df[df.iloc[:,0]=='Odour2.3'].index.values.astype(int)[0]  
    # select the data between set indexes
    df_23 = df.query('index == 10')                          
                          
    
   
    # merge the data together
    df_all = pd.merge(df_21, df_22, left_on = ['filename'], right_on = ['filename'], how = 'left')
    df_all = pd.merge(df_all, df_23, left_on = ['filename'], right_on = ['filename'], how = 'left')
...
# write to csv
log.to_csv(r'C:\Users\Sevda\Downloads\ml_paper\c9-tdo43-reassessment\combined\extra3\manual scores\all manual scores\combined\odour2_by order.csv', index = False)