...
>
>After extracting the Data folder change the working directory/input/output folder in each .py or .rmd file to the correct one for you.
...
* **DLC and video data prep for SimBA analysis** - contains scripts for keeping only cotton bud frames from videos and matching the DLC tracking files to the cleaned up videos. Video and DLC prep folder contains .py script that removes video frames without cotton bud based on DLC confidence scores - you need to have your video files and DLC output in the same folder with matching ids, together with the script file, adjust the column location for the cotton bud data based on your set up. DLC prep contains a single example DLC output file and a script to remove for frames without cotton bud and to remove cage output afterwards.
...
* **Simba training** - contains features importance logs, classification reports, learning curves and meta data files for the SimBA model that was trained to analyse the validation dataset presented in the paper.

* **Simba output - 1 second time bins** - Time_bins_ML_results.csv shows the raw output from the SimBA validation analysis of unseen videos of estimated time spent sniffing at every 1 s time bin from each validation video. time_binning_9.py compiles these 1 s time bins into 9 values - one for each cotton bud presentation according to the duration of each individual video.

* **Manual scores** - all manual scores.zip folder contains all manual scores from each video used in the unseen validation analysis presented in the paper, as well as the manual score files for the comparison of two manual scorers from Figure 3, this folder also contains .py scripts for collating individual files together, when using these, change the input path to the one relevant to you and the output path to ~/combined/filename.csv. SimBA-training-SVCtoSolomon.zip contains the works flow used to change the SimpleVideoCoder manual analysis output to Solomon out put which is recognised by SimBA and was used for SimBA training.

  Analysis pipeline:

<img width="1333" height="750" alt="Analysis pipeline" src="https://github.com/user-attachments/assets/8b8561ed-9163-434a-b51b-3c823d30b7d8" />
